...
const LibraryLandingPage = ({ folders }: LibraryLandingPageProps) => {
  const libraryNav = useLibraryNavigationStore();
  const { categories, totalTemplatesCount } = useLibraryLandingData(folders);

  const language = libraryNav.languageFolder?.title ?? "";
  const countriesCount = libraryNav.countriesCount;
...
export default observer(LibraryLandingPage);
